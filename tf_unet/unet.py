...
class Trainer(object):
	...
	def _get_optimizer(self, training_iters, global_step):
		if self.optimizer == "momentum":
			...
			learning_rate = self.opt_kwargs.pop("learning_rate", 0.001)
			self.learning_rate_node = tf.Variable(learning_rate, name="learning_rate")
			optimizer = tf.train.GradientDescentOptimizer(learning_rate=self.learning_rate_node, **self.opt_kwargs).minimize(self.net.cost, global_step=global_step)
			...
			momentum = self.opt_kwargs.pop("momentum", 0.2)
			self.learning_rate_node = tf.Variable(learning_rate, name="learning_rate")
			optimizer = tf.train.RMSPropOptimizer(learning_rate=self.learning_rate_node, 
												decay=decay_rate, 
												momentum=momentum).minimize(self.net.cost, global_step=global_step)
		...
		with tf.Session() as sess:
			...
			for epoch in range(epochs):
				...
				for step in range((epoch * training_iters), ((epoch + 1) * training_iters)):
					...
					if step % display_step == 0 and str(loss) != 'inf' and str(loss) != 'nan' and str(loss) != 'NaN':
						self.output_minibatch_stats(sess, summary_writer, step, batch_x, util.crop_to_shape(batch_y, pred_shape))

					total_loss += loss

					print(f'Epoch {epoch} step ({step}/{(epoch+1)*training_iters}) loss: {loss}')
